 ...
 * @version 5.0.8 ($Revision: 213 $), $Date: 2013-11-17 $
 */
public abstract class QuantityFactory<Q extends Quantity<Q>>  {
	...
	public static <Q extends Quantity<Q>>  QuantityFactory<Q> getInstance(final Class<Q> type) {
         ...
         if (!type.isInterface()) {
        	 if (type != null && type.getInterfaces() != null & type.getInterfaces().length > 0) {
	            ...
	            if (!PhysicsMeasurement.class.isAssignableFrom(type2))
	            ...
	            INSTANCES.put(type2, factory);
                 ...
                 if (factory != null) return factory;
                 if (!PhysicsMeasurement.class.isAssignableFrom(type))
                     // This exception is not documented because it should never happen if the
                     // user don't try to trick the Java generic types system with unsafe cast.
                     throw new ClassCastException();
     ...
     * a default implementation for every {@link PhysicsMeasurement} sub-types.
     *
     * @param <Q> The type of the quantity
     */
        ...
        public Object invoke(final Object proxy, final Method method, final Object[] args) {
            ...
            if (name.equals("doubleValue")) { // Most frequent.
	          ...
	          if ((toUnit == unit) || (toUnit.equals(unit)))
                ...
                if (!(obj instanceof PhysicsMeasurement))
                    return false;
                final PhysicsMeasurement<Q> that = (PhysicsMeasurement<Q>) obj;
                if (!unit.isCompatible((PhysicsUnit<?>) that.unit()))
                    return false;
                return value.doubleValue() == (that).doubleValue(unit);
            } else if (name.equals("compareTo")) {
                final PhysicsMeasurement<Q> that = (PhysicsMeasurement<Q>) args[0];
