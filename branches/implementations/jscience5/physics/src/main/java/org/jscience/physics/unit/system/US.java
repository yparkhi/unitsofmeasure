...
import static org.jscience.physics.unit.system.SIPrefix.MICRO;

import org.jscience.physics.unit.AbstractSystemOfUnits;
...
public final class US extends AbstractSystemOfUnits {
	...
	public static final Unit<Area> SQUARE_FOOT = addUnit(new ProductUnit<Area>(
			(PhysicsUnit<?>) FOOT.multiply(FOOT)));
	...
	private static <U extends Unit<?>> U addUnit(U unit, String name) {
		if (name != null && unit instanceof PhysicsUnit) {
			PhysicsUnit<?> aUnit = (PhysicsUnit<?>)unit;
			//aUnit.setName(name);
